...
class Sprite extends ImageView {
	...
	public Sprite(String fileName,int numRows, int numCols) {
		...
		if(fileName == "file:sprites/idle.png"){
			try {
				setUpAnimation(1);
			} catch (Exception e) {
				e.printStackTrace();
		...
		this.imageIndex = 0;
		Image currentImage = this.sprites[this.imageIndex];
	...
	private void setUpAnimation() {
		...
		animation = new Timeline(new KeyFrame(Duration.millis(1500/sprites.length), e -> nextFrame() ));
		animation.setCycleCount(Timeline.INDEFINITE);
		animation.play();
	}

	private void setUpAnimation(int one) {
		// just to have extra call for idle as slower

		animation = new Timeline(new KeyFrame(Duration.millis(3500/sprites.length), e -> nextFrame() ));
		animation.setCycleCount(Timeline.INDEFINITE);
		animation.play();
	}
